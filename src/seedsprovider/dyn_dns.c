...
// Keep poling DNS server for the TXT record with seeds, same format as for
// Florida seeds
//
//
// DYNOMITE_DNS_TXT_NAME=_dynomite.yourhost.com src/dynomite -c
// conf/dynomite_dns_single.yml -v 11
//
...
uint8_t dns_get_seeds(struct context *ctx, struct mbuf *seeds_buf) {
  ...
  if (!_env_checked) {
    ...
    if (dnsType != NULL) {
      if (strcmp(dnsType, "A") == 0) queryType = T_A;
    }
  }
  ...
  if (r == -1) {
    log_debug(LOG_DEBUG, "DNS response for %s: %s", dnsName,
              hstrerror(h_errno));
    return DN_NOOPS;
  }
  if (r >= sizeof(buf)) {
    log_debug(LOG_DEBUG, "DNS reply is too large for %s: %d, bufsize: %d",
              dnsName, r, sizeof(buf));
    return DN_NOOPS;
  }
  ...
  if (ns_initparse(buf, r, &m) == -1) {
    log_debug(LOG_DEBUG, "ns_initparse error for %s: %s", dnsName,
              strerror(errno));
    return DN_NOOPS;
  }
  int i;
  ...
  for (i = 0; i < na; ++i) {
    ...
    if (s[0] >= ns_rr_rdlen(rr) && (strcmp(dnsType, "A") != 0)) {
      log_debug(LOG_DEBUG, "invalid length for %s: %d < %d", dnsName, s[0],
                ns_rr_rdlen(rr));
      return DN_NOOPS;
    }
